...
export function buildPersonalWeekPlan(recipes: PersonalMenuRecipe[], input: { pantry: string[]; favoriteIngredients: string[]; goal: string; allergies: string[] }) {
  ...
  const ordered = (safeRecipes.length ? safeRecipes : recipes).slice().sort((a, b) => score(b) - score(a));
