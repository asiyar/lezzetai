...
type ReceiptResult = { ingredients: { name: string; category: string; confidence: "Yüksek" | "Orta"; quantity: number; unit: StockUnit; quantityConfidence: "Yüksek" | "Orta" | "Düşük" }[]; safetyNote: string };

export default function ScanReceiptScreen() {
  const [imageUri, setImageUri] = useState<string | null>(null);
  const [result, setResult] = useState<ReceiptResult | null>(null);
  const { addPantryItems } = useLezzet();
  const scan = trpc.chef.scanReceipt.useMutation({ onSuccess: setResult });
  ...
  const addAll = () => {
    ...
    Alert.alert("Kiler güncellendi", `${result.ingredients.length} ürün yaklaşık miktarıyla kilere eklendi. Son kullanma tarihlerini Kiler ekranından ekleyebilirsin.`);
