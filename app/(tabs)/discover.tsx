...
import { getCategoriesForLocale, getRecipesForLocale } from "@/lib/lezzet-data";
import { useLezzet } from "@/lib/lezzet-context";
import { getCuisineProfile } from "@/lib/cuisine-locale";

export default function DiscoverScreen() {
  const { favorites, toggleFavorite, profile } = useLezzet();
  ...
  const regionalRecipes = useMemo(() => getRecipesForLocale(profile.locale), [profile.locale]);
    ...
    <ScreenContainer className="flex-1" containerClassName="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.list}>
          ...
          {filtered.length === 0 ? <View style={styles.empty}><IconSymbol name="magnifyingglass" size={26} color="#6B756F" /><Text style={styles.emptyTitle}>No local recipe found for this search.</Text><Text style={styles.emptyText}>Try another ingredient or cuisine category.</Text></View> : null}
        </View>
      </ScrollView>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  content: { padding: 20, paddingBottom: 32, gap: 18 }, eyebrow: { color: "#6B756F", fontSize: 11, fontWeight: "800", letterSpacing: 1.1 }, title: { color: "#1E2521", fontSize: 30, lineHeight: 36, fontWeight: "800", letterSpacing: -0.8, marginTop: 2 }, regionLabel: { color: "#52705E", fontSize: 11, fontWeight: "800", marginTop: 5 },
